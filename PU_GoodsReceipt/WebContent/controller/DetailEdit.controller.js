sap.ui.define([
		...
		var PageController = Controller.extend("z_goods_receipt.controller.DetailEdit",{
			formatter : formatter,
		...
		handleECaddButtonPressed: function(oEvent){
			if (!this.pressDialog) {
				this.pressDialog = sap.ui.xmlfragment(this.getView().getId(),
						"z_goods_receipt.view.fragment.ECitems",
						this);
		...
		onECselection: function(oEvent) {
			...
			if (!this.pcItemDetailDialog) {
				this.pcItemDetailDialog = sap.ui.xmlfragment(
						"z_goods_receipt.view.fragment.ECeditItems",
						this);
				// to
