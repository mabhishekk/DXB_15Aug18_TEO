sap.ui.define([
		...
		var PageController = Controller.extend("providenta.fi.rewards.controller.DetailEdit",{
			formatter : formatter,
			...
			onInit: function() {
				...
				this.getOwnerComponent().getRouter().getRoute("RewardsAppreciationDetailsEdit").attachPatternMatched(this._onRouteMatched, this);
			},
			
			_onRouteMatched: function(oEvent) {
				this._RewardsNo = oEvent.getParameter("arguments").sId;
				...
				var sFilter = "Postingnumber eq '" + this._RewardsNo + "'";
...
//						}))
		...
		handleERewardsSave: function(oEvent){
		...
		_SaveRewards: function(oEvent) {
			...
			oModel.create("/RewardsSet",temObj,{
				...
				success:function(oData){
					...
					var msg = that.getResourceBundle().getText("RaUpdateSuccess",[sPostingNumber]);
...
//									sap.m.MessageToast.show("Submitted");
		...
		_CancelYes: function(oEvent) {
			...
			this.getOwnerComponent().getRouter().navTo("RewardsAppreciationDetailsDisplay", {sId: sPostingNumber},	!Device.system.phone);
		},
		onNavBack:function(){
		...
		onFileUpload: function(oEvent){
	        ...
	        reader.onload      = (function(theFile) {
	        	return function(evt) {
		            ...
		            tablObj.Doknr         = doknr;
		            tablObj.Postingnumber = that._RewardsNo;
		...
		onSelection: function(oEvent) {
			...
			if (!this.ItemDetailDialog) {
				this.ItemDetailDialog = sap.ui.xmlfragment(
						"providenta.fi.rewards.view.fragment.ECeditItems",
						this);
				// to
		...
		handleECaddButtonPressed: function(oEvent) {
			if (!this.pressDialog) {
				this.pressDialog = sap.ui.xmlfragment(this.getView().getId(),
						"providenta.fi.rewards.view.fragment.ECitems",
						this);
...
//			this.getView().byId("id_ECexpSet").getValue();
		...
		onFileDelete: function(oEvent){
	    	  var ofdelObj  = oEvent.getParameter('listItem').getBindingContext().getObject();
			  var sdelPath  = oEvent.getParameter('listItem').getBindingContext().getPath();
			  this._deleteFile(oEvent, ofdelObj, sdelPath, this._RewardsNo);
	    }
