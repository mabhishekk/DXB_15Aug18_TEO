sap.ui.define([
		...
		var PageController = Controller.extend("z_inbox.controller.displayPRcycle.goodsReceipt",{
			formatter : formatter,
			...
			onInit : function() {
				...
				this.getOwnerComponent().getRouter().getRoute("GRdisplay").attachPatternMatched(this._onRouteMatched, this);
			},
...
//				this.prNumber = this.getOwnerComponent().getComponentData().startupParameters.id[0];
				...
				this._sId      = "/GRHEADDISPSet('"+  oEvent.getParameter("arguments").id +  "')";
			...
			_BindServiceOrderTable: function(sMatdocno, sMatdocitem) {
				...
				oElement.bindItems({
					path : "poService>/GRINVSERVICESSet",
					filters: new Filter(aFilters, true),
					template : oTemplate
				});
			},	
			...
			_BindMaterialTable: function(sMatdocno) {
				...
				aFilters.push( new Filter("Matdocno", "EQ", sMatdocno) );
				oElement.bindItems({
					path : "poService>/GRITEMDISPFINALSet",
					filters: new Filter(aFilters, true),
					template : oTemplate
				});
				this._setDate(sMatdocno);
