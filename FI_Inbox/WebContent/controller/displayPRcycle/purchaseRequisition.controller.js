sap.ui.define([
	...
	return Controller.extend("z_inbox.controller.displayPRcycle.purchaseRequisition",	{
		formatter : formatter,
		onInit : function() {
			...
			this.getOwnerComponent().getRouter().getRoute("PRdisplay").attachPatternMatched(this._onRouteMatched,this);
		},
...
onApproveClose : function(oEvent) {
	oEvent.getSource().getParent().close();
		...
		handlePRIconTabBar: function(oEvent){
			this.getOwnerComponent().getRouter().navTo("QRdisplay", {id : this.sId, instId : 'Display'});
		}
