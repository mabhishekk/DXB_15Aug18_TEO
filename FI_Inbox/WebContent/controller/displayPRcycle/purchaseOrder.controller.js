sap.ui.define( [
		"jquery.sap.global",
		"z_inbox/controller/displayPRcycle/BaseController", 
		"sap/ui/Device",
	...
	return Controller.extend("z_inbox.controller.displayPRcycle.purchaseOrder", {
		onInit: function() {
//			if (!jQuery.support.touch) { this.getView().addStyleClass("sapUiSizeCompact");
			...
			this.getOwnerComponent().getRouter().getRoute("POdisplay").attachPatternMatched(this._onRouteMatched,this);
...
//	},
	...
	_onRouteMatched:function(oEvent){
		...
		Payterms.setModel(this.getOwnerComponent().getModel("poService"));
		Payterms.bindItems({
...
//		
	...
	onPOEditSave: function(){
		this.getView().setBusy(true); 
		var poModel = this.getOwnerComponent().getModel("poService"); 
		var itemsData = [];
...
//		itemsData = this.lModel.getProperty('/mainSet/navigtoitems/results');
	...
	onPOItemaddPress : function(oEvent) {
		if (!this.prCommDialog) {
			this.prCommDialog = sap.ui.xmlfragment(this.createId("itemsFragment"),"z_inbox.view.fragments.POItemAdd",this);
...
//		this.cntxt .PreqPrice = ""
		...
		if (!this.prItemDetailDialog) {
			this.prItemDetailDialog = sap.ui.xmlfragment(this.createId("itemDetail"),"z_inbox.view.fragments.POItemDetail",this); 
			// to
...
//			 sBtn =  sap.ui.core.Fragment.byId(fragmentId,"mtrl");
		...
		matrnTaxCode.setModel(this.getOwnerComponent().getModel("poService"));
		...
		matrnGLCode.setModel(this.getOwnerComponent().getModel("poService"));
