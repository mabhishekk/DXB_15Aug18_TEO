sap.ui.define([
		...
		var PageController = Controller.extend("z_inbox.controller.creditCard.creditCardEdit",{
			formatter : formatter,
		...
		onInit : function() {
			...
			this.getOwnerComponent().getRouter().getRoute("CreditCardEdit").attachPatternMatched(this._onRouteMatched, this);
		},
		
		_onRouteMatched: function(oEvent) {
			this.getView().setModel(this.getView().getModel("fiService"));
			this._sWorkItemId  = oEvent.getParameter("arguments").instId;
			this._CreditCardNo = oEvent.getParameter("arguments").id;
		...
		onSelection: function(oEvent) {
			...
			if (!this.ItemDetailDialog) {
				this.ItemDetailDialog = sap.ui.xmlfragment(
						"z_inbox.view.creditCard.CCeditItems",
						this);
				// to
...
//			oEvent.getSource().getParent().close();
		...
		handleCCaddButtonPressed: function(oEvent) {
			if (!this.pressDialog) {
				this.pressDialog = sap.ui.xmlfragment(this.getView().getId(),
						"z_inbox.view.creditCard.CCitems",
						this);
		...
		onOpenDetailDialItem:function(oEvent){
			...
			expSet.setModel(this.getView().getModel("fiService"));
			...
			FGcurr.setModel(this.getView().getModel('fiService'));
...
//			this.getView().byId("idCCexpSet").getValue();
		...
		handleCCSaveAndSubmit: function(oEvent) {
			...
			var sItemMsg   = this.getResourceBundle().getText("CCCcEmptyItemError");
			if(sListItems === 0){
		...
		_onCCsaveSubmit: function(oEvent) {
			...
			oModel.create("/CreditHeaderSet",temObj,{
				...
				success:function(oData){
					...
					var msg = that.getResourceBundle().getText("CCCcUpdateSuccess", [sPostingNumber]);
					jQuery.sap.require("sap.m.MessageBox");
					sap.m.MessageBox.success(msg, {
						onClose: function(){
							that.getOwnerComponent().getRouter().navTo("CreditCard", {id : sPostingNumber,instId : wiid});
						}
					});
			...
			if (!this.CancelDialog) {
				this.CancelDialog = new sap.m.Dialog({
							...
							content : new sap.m.Text({
										text : this.getResourceBundle().getText("CCCancelEdit")
									}),
...
//									sap.m.MessageToast.show("Submitted");
		...
		_CancelYes: function(oEvent) {
			...
			if (sPreviousHash !== undefined) {
				...
				this.getOwnerComponent().getRouter().navTo("CreditCard", {id: sPostingNumber, instId:sWiId }, !Device.system.phone);
			} else {
				// There is no history!
