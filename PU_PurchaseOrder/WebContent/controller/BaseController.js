sap.ui.define([
	...
	return Controller.extend("poApp.controller.BaseController", {
		...
		onValueCurrecyChange: function(oEvent){
...
//			var Vatvalue      = Number(sap.ui.core.Fragment.byId(fragmentId,"matrnVAT").getValue());
			...
			var Discount      = Number(sap.ui.core.Fragment.byId(fragmentId,"matrnDiscount").getValue())
			var sChangedField = oEvent.getSource().getId().split('--')[2];
			if(sChangedField !== "matrnDiscount"){
				Discount      = Value * Discountvalue / 100;
				sap.ui.core.Fragment.byId(fragmentId, "matrnDiscount").setValue(Discount.toFixed(2));
			}
		...
		liveDiscountChange: function(oEvent){
			var viewID         = this.getView().getId();
			var fragmentId1    = oEvent.getParameter("id").split("--")[1];
			var fragmentId     = viewID + "--" +fragmentId1;
			var sDiscountValue = oEvent.getSource().getValue();
			var perUnitPrice   = sap.ui.core.Fragment.byId(fragmentId,"matrnVal").getValue();
			var sDiscountPer   = (Number(sDiscountValue) / Number(perUnitPrice) * 100 ).toFixed(2);
			sap.ui.core.Fragment.byId(fragmentId,"matrnDiscVal").setValue(sDiscountPer);
...
// cost center
	...
	prUrgentStatus:function(oParm){
		...
		handleIconTabBarSelect:function(oEvent){
			...
			var extArr = ["PR","QR","QC"];
			var sPRnumber = this.lModel.getProperty('/mainSet/Preq_No');
			if(sPRnumber === undefined){
				sPRnumber = this.lModel.getProperty('/PrDetail/Banfn');
			}
			var oCrossAppNav = sap.ushell.Container.getService("CrossApplicationNavigation");
			if(icKey == "PR"){
//				this.getOwnerComponent().getRouter().navTo(icKey, {id: this.sId});
				var shlHash = "z_pr-change&/PR-"+sPRnumber+"/Edit"
				...
				var shlHash = "z_pr-change&/PR-"+sPRnumber+"/quotation"
				var hrefForProductDisplay  =  oCrossAppNav.toExternal({
					  target : { shellHash : shlHash }
				});
				...
				var shlHash = "z_pr-change&/PR-"+sPRnumber+"/quotComparision"
				var hrefForProductDisplay  =  oCrossAppNav.toExternal({
					  target : { shellHash : shlHash }
				});
		...
		onOpenDetailDialItem : function(oEvent) { 
			...
			var sCurrency          = sap.ui.core.Fragment.byId(fragmentId,	"matrnCurr").getSelectedKey();
			Value			       = Value*Quanity;
			var Discount           = (Value * Discountvalue / 100).toFixed(2);
			...
			sap.ui.core.Fragment.byId(fragmentId,"idSubTotalCurr").setSelectedKey(sCurrency);
			sap.ui.core.Fragment.byId(fragmentId,	"matrnNVCurr").setSelectedKey(sCurrency);			
			sap.ui.core.Fragment.byId(fragmentId,"matrnDiscount").setValue(Discount);
...
//			});
		...
		onSaveDetailItem : function(oEvent) {
			...
			if (PoType === '1'){
				...
				var netValue        = sap.ui.core.Fragment.byId(this.getView().createId("itemDetail"),"matrnVal").getValue(); 
...
//			},
			...
			onChangeCostCenter:function(oEvent){
				...
				var matrnGLCode = sap.ui.core.Fragment.byId(fragmentId,"matrnGLCode");;
				matrnGLCode.setModel(this.getView().getModel('prm'));
				matrnGLCode.setModel(this.lModel,"lModel");
