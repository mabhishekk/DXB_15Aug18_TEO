sap.ui.define([
	...
	return Controller.extend("poChangeApp.controller.BaseController", {
...
//					this._bindItemForm(sIndex);
				...
				success:function(oData, response){
					if (response.statusText === "OK"){
						...
						if(this.line <  this.oItemLength){
							this._loadSO(this.line, this.sPO, this.tempIndex);
							this.getView().setBusy(false);
						}else{
		...
		_TotalCalculation: function(aData){
			var aItemData   = aData.navtoitem.results,
				aItemLength = aItemData.length,
				i=0, CP=0, DP=0, VP=0, SP=0;     // Cost Price, Discount Price, VAT Amount, Selling Price
			for(i=0; i<aItemLength; i++){
				var quantity      = Number(aItemData[i].Quantity);
				var originalValue = Number(aItemData[i].NetPrice);
				var discountValue = Number(aItemData[i].Discountval);
				var vat           = Number(aItemData[i].Vatvalue);
				CP                = CP + (quantity*originalValue);
				DP                = DP + (quantity*originalValue*discountValue/100);
				VP                = VP + ((quantity*originalValue)*(1 - (discountValue/100))*(vat/100));
				SP                = SP + ((quantity*originalValue)*(1 - (discountValue/100))*(1 + (vat/100)));
			}
			CP = (Math.round(CP * 100) / 100).toString();
			DP = (Math.round(DP * 100) / 100).toString();
			VP = (Math.round(VP * 100) / 100).toString();
			SP = (Math.round(SP * 100) / 100).toString();
			this.tempModel.setProperty('/headerData/Subtot', CP);
			this.tempModel.setProperty('/headerData/Totdiscount', DP);
			this.tempModel.setProperty('/headerData/Vatvalue', VP);
			this.tempModel.setProperty('/headerData/Totalamt', SP);
		}
	});
});