//PR User Create controller 
sap.ui.define([ 
    "userPr/controller/BaseController", 
    "sap/ui/core/routing/History",
    "sap/ui/Device", 
    'sap/m/MessageBox',
    'sap/m/MessageToast', 
    "userPr/model/formatter",
    'sap/m/Dialog',
    'sap/m/Button',
    'sap/m/Text'
  ], function(Controller, History, Device, MessageBox, MessageToast,formatter, Dialog, Button, Text) {

    return Controller.extend("userPr.controller.DetailCreate", { 
        formatter : formatter,
        onInit : function() {
...
//          this.imageLoading(this.getOwnerComponent().getMetadata().getComponentName());
        ...
        onDetailItemPress : function(oEvent) {
          ...
          if (!this.prItemDetailDialog) {
            this.prItemDetailDialog = sap.ui.xmlfragment(this.createId("itemDetail"),"userPr.view.fragments.PRItemDetailCommercial",this); // to
...
//          });
          ...
          var aData = jQuery.ajax({
            ...
            success : function(data,textStatus, jqXHR) {
              that.getView().byId("idRequester").setValue(data.id);
            }
          });
...
//                                  this._apprvYes(oEvent);
          ...
          onPRaddPress : function(oEvent) {
            if (!this.prCommDialog) {
              this.prCommDialog = sap.ui.xmlfragment(this.createId("itemsFragment"),"userPr.view.fragments.PRCommercial",this);
          ...
          _onRouteMatched : function(oEvent) {
            ...
            var tbl = this.getView().byId("pr_cr_item");
            var tblPT = this.getView().byId("id_payTerms");
            var id_addInfo = this.getView().byId("id_addInfo");
            var id_payment = this.getView().byId("id_payment");
...
//                this.validation();
                ...
                if (lSet.appRs6) {
                  poData.navigtoapproval[5].Reason = lSet.appRs6;
                }
                if (lSet.appRs7) {
                  poData.navigtoapproval[6].Reason = lSet.appRs7;
                }
...
*/
              ...
              intGnrlValidation:function(icKey){
                var ptData      = this.getView().getModel('lModel').getProperty("/mainSet");
                var requestType = this.byId("L4").getSelectedIndex();
                if (requestType != 1){
                  var zLifnr  = ptData.Lifnr ? true: false;
                } else {
                  var zLifnr  = true;
                }
...
//                var zptDat = ptData.navigtoitems.length == 0 ? false:true;
                ...
                return  (zDat & zreBrf & zLifnr)==1?true:false;
